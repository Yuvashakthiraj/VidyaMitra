...
async function getS3Stats(env: Record<string, string>) {
  ...
  // Cost estimate: first 5GB/month free in standard tier (simplified)
  const billableGB = Math.max(0, sizeGB - 5);
  ...
  return {
    ...
    costs: {
      ...
      freeTierNote: 'First 5 GB/month free. PUT/GET: 2K PUT + 20K GET free/month.',
    },
  };
}
...
async function getLambdaStats(env: Record<string, string>) {
  const creds = makeAwsCreds(env);
  const region = getRegion(env);
  ...
  const billableRequests = Math.max(0, monthlyInvocations - PRICING.lambda_free_requests);
  ...
  const billableGbSec = Math.max(0, monthlyGbSec - PRICING.lambda_free_gb_sec);
  const computeCost = billableGbSec * PRICING.lambda_gb_sec;

  // Textract pages ~ 1 per Lambda invocation
  const textractPages = invocations;
  const billableTextractPages = Math.max(0, textractPages - PRICING.textract_free_pages);
  ...
  return {
    ...
    costs: {
      ...
      freeTierNote: 'Lambda: 1M req + 400K GB-sec/month free. Textract: 1K pages/month free.',
    },
  };
}
...
async function getSNSStats(env: Record<string, string>) {
  const creds = makeAwsCreds(env);
  const region = getRegion(env);
  ...
  try {
    ...
    const billableMessages = Math.max(0, monthlyMessages - PRICING.sns_free_email);
    ...
    return {
      ...
      costs: {
        ...
        freeTierNote: 'First 1,000 email notifications/month free. $2.00 per 1M after that.',
      },
    };
  } catch (err: any) {
    ...
    return {
      ...
      costs: {
        ...
        freeTierNote: 'First 1,000 email notifications/month free.',
      },
    };
  }
}
...
export function registerAwsUsageRoutes(
  ...
  server.middlewares.use('/api/aws/usage', async (req: any, res: any, next: any) => {
    ...
    const [s3, lambda, sns] = await Promise.all([
      getS3Stats(env).catch(err => ({ error: err?.message ?? 'Failed', service: 'S3', status: 'error' })),
      getLambdaStats(env).catch(err => ({ error: err?.message ?? 'Failed', service: 'Lambda + Textract', status: 'error' })),
      getSNSStats(env).catch(err => ({ error: err?.message ?? 'Failed', service: 'SNS', status: 'error' })),
    ]);
